...
/**
 * This class consists of JUnit tests for deploying Application D on Production Server #1
 * with Habitat4J.
 * 
 ...
 * @version $Id: ApplicationDonProduction1TestCase.java,v 1.9 2004/08/20 11:00:03 cvs Exp $
 */
public class ApplicationDonProduction1TestCase extends Habitat4JBaseTestCase implements Habitat4JConstants {
	...
	public void testServerIdentity() {
		try {
			...
			assertEquals("server-prod-1",ServerIdentityManager.getInstanceName());
			assertEquals("Production Application Server #1",ServerIdentityManager.getInstanceDescription());
	...
	public void testLDAPonProductionServer1() {
		try {
			doLoadFromResource();
			...
			assertEquals(ldap.getAuthUser(),"prodacct1");
			assertEquals(ldap.getAuthPass(),"prodpass1");
	...
	public ApplicationDonProduction1TestCase(String arg0) {
		super(arg0);
	}
}
