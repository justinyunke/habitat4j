...
/**
 * This class consists of JUnit tests for deploying Application D on Production Server #2 
 * with Habitat4J.
 * 
 ...
 * @version $Id: ApplicationDonProduction2TestCase.java,v 1.9 2004/08/20 11:00:03 cvs Exp $
 */
public class ApplicationDonProduction2TestCase extends Habitat4JBaseTestCase implements Habitat4JConstants {
	private static final String APPLICATION_NAME			= "ApplicationD";
	private static final String SERVER_NAME					= "servers/production/app-server-2/";
	
	public static final String[] TEST_LIST = {
		"testServerIdentity",
		"testLDAPonProductionServer2"
	};
	...
	public void testServerIdentity() {
		try {
			...
			assertEquals("server-prod-2",ServerIdentityManager.getInstanceName());
			assertEquals("Production Application Server #2",ServerIdentityManager.getInstanceDescription());
	...
	public void testLDAPonProductionServer2() {
		try {
			doLoadFromResource();
			...
			assertNotNull(ldap);
			assertEquals(ldap.getHost(),"ldap-prod-2.bogus.productivity.org");
			...
			assertEquals(ldap.getAuthUser(),"prodacct2");
			assertEquals(ldap.getAuthPass(),"prodpass2");
	...
	public ApplicationDonProduction2TestCase(String arg0) {
		super(arg0);
	}
}
